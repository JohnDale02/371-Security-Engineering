...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        for i in subblocks:
            for j in self.compute_s_box(i, round):
                result[a] = int(j)
                a += 1
            round = round+1
    ...
    def compute_s_box(self,block,round):
        ...
        Row1 = binIn[0]
        Row2 = binIn[-1]
        Row = int(str(Row1) + str(Row2),2)
        Col1 = binIn[1]
        Col2 = binIn[2]
        Col3 = binIn[3]
        Col4 = binIn[4]
        Column = int(str(Col1) + str(Col2) + str(Col3)+ str(Col4),2)

        currSBox = S_BOX[round]
